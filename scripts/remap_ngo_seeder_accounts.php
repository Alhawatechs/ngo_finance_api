...
/**
 * Outputs old_code => new_code map for NGO getAccounts() rows using CoaDottedMigrationService.
 * Run from backend: php scripts/remap_ngo_seeder_accounts.php
 */
...
$block = $m[1];
preg_match_all("/'code' => '(\d+)'[^\\]]*'parent_code' => '(\d+)'/", $block, $matches, PREG_SET_ORDER);
foreach ($matches as $match) {
    $code = $match[1];
    $parent = $match[2];
    $add($code, 'expense', $parent);
}
preg_match_all("/'code' => '(\d+)'[^\\]]*'type' => '(asset|liability|revenue|expense|equity)'[^\\]]*'parent_code' => '(\d+)'/", $block, $matches2, PREG_SET_ORDER);
foreach ($matches2 as $match) {
    // already added by first pass if duplicate
}
// Re-parse with full type
$rows = [];
$id = 1;
$byCode = [];
$add = function (string $code, string $type, ?string $parentCode) use (&$rows, &$id, &$byCode) {
...
echo json_encode($oldToNew, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
